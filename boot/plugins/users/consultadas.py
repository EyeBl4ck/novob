...
@Client.on_message(filters.command(["consulta", "consultada"]))
@Client.on_callback_query(filters.regex("^consulta$"))
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
           [
                InlineKeyboardButton(
                    "🏳️ Pesquisar Consul",
                    switch_inline_query_current_chat="consulta_buy RENNER",
                ),
                InlineKeyboardButton(
                    "🏳️ Pesquisar Banco",
                    switch_inline_query_current_chat="consulta_banco A",
                ),
            ],
            [
            InlineKeyboardButton(
                    "🏳️ Pesquisar Bandeira",
                    switch_inline_query_current_chat="consulta_bandeira VISA",
                ),
            ],
            [
            InlineKeyboardButton(
                    "🏳️ Pesquisar por cc",
                    switch_inline_query_current_chat="consulta_cc 5",
                ),
            ],
[               InlineKeyboardButton("🔸 Atualizar",callback_data="consulta"),
    ...
    counts = cur.execute(
        f"SELECT COUNT(*) FROM consulta"
    ).fetchone()

    stock = (
        "\n".join([f"<b>{it[0]}</b>: {it[1]}" for it in ccs])
        or "<b>⚠️ Sem consultadas no momento</b>"
    )
    ...
    for it in ccs:
    	rt = cur.execute(
    	f"SELECT limite, preco, anjo, token, cc, bincc, senha,mes, ano, cvv, cpf, telefone, nome, added_date,nomebanco FROM consulta WHERE nomebanco = ? ORDER BY RANDOM() LIMIT 20",
    	[it[0]],).fetchall()
    	print(rt)
    	print(rt[0])
...
#   ('item4', 'categoria2', 'descricao4')]}
    ...
    start_message = f"""⚜<b> Olá {m.from_user.first_name}, seja bem vindo a aba de consultadas use /pix para realizar uma tranferencia em nossa store.</b>
<a href='https://cdn.dribbble.com/users/2657768/screenshots/15420992/media/7854e227fa9c24f716be63d4a2f35fd9.mp4'>&#8204</a>

<b>✅ | LISTA COM {counts[0]} CONSULTADAS</b>

<b>Preço  -     Consultadas     -  Nome  -  Limite</b>

<b>{obter_resultados(resultados)}</b>

<b>💳 Consultadas Disponiveis - </b>\n\n{stock}{total}

<b>🎗 Leia as regras e termo de troca se não for de acordo não compre 🎗</b>
<b>⌛️ 15 Minutos para acesso
