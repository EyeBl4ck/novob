...
@Client.on_callback_query(filters.regex(r"^procurar$"))
async def comprar_cc_list(c: Client, m: CallbackQuery):
    kb = InlineKeyboardMarkup(
     ...
     #           ),
                InlineKeyboardButton(
                    "🔐 Buscar bin",
                    switch_inline_query_current_chat="buscar_bin",
                ),
            ],
        #    [
        #        InlineKeyboardButton(
...
async def search_cc(c: Client, m: InlineQuery):
    ...
    for number, month, year, value in rt:
        ...
        price = await get_price("bin", number[0:4])

        base = f"""Cartão: {number[0:4]}********
Cvv: ***"""

        base_ml = f"""<b>Cartão:</b> <i>{number[0:4]}********</i>
<b>Cvv:</b> <i>***</i>
        ...
        kb = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(
                        text="✅ Comprar",
                        callback_data=f"comprar_cc bin '{number[0:4]}'",
                    )
                ]
            ]
        )
